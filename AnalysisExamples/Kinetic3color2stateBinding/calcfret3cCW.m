function varargout = calcfret3cCW(inputdata, donorChans, acceptChans, acceptChans2, varargin)
%
...
elseif ischar(inputdata) % if inputdata is filename, load and analyze each file.
    ...
    for fileindex=1:numfiles
        ...
        [OneFrethist, OneEvents, OnefretBurst] = calcfretsub(data, ...
            donorChans, acceptChans, acceptChans2, gamma, xAxis, donorleak);
...
fretBurst = [burstnumber, [effBurst1 effBurst2], donorBurst+accptBurst+accpt2Burst, donorBurst, accptBurst];

% build FRET histogram, 
frethist = histc(fretBurst(:,2:3), xAxis);
