function [errorparams logmlh1 bic]=mlhDiffNTR3cDA1CWBindbkgerrorC(initparams,frburstdata,cumindex,indexone,Diffparams,numC,drvintv,drvpnt)
...
for kk=1:length(initparams)
    for nn=1:length(drvintvs)
        ...
        if nstate > 2
            peqtemp=initparamone(5*nstate-2:6*nstate-4);
            ...
            initparamone(5*nstate-2:6*nstate-4)=frn;
        end
...
logmlh=mlhDiffNTR3cDA1CWBindbkg_MT(initparammat,LUbounds,frburstdata,cumindex,indexone,Diffparams{1},numC);
